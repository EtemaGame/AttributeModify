...
public record S2CEditorCatalogPacket(String payload) {
    ...
    public static S2CEditorCatalogPacket decode(FriendlyByteBuf buf) {
        return new S2CEditorCatalogPacket(buf.readUtf(MAX_PAYLOAD_SIZE));
    }

    public static void handle(S2CEditorCatalogPacket packet, Supplier<NetworkEvent.Context> contextSupplier) {
        NetworkEvent.Context context = contextSupplier.get();
        context.enqueueWork(() -> EditorClientState.setLatestCatalogJson(packet.payload()));
